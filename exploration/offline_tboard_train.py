...
parser.add_argument("--enssched", choices = ['const', 'lin', 'exp', 'cos'], default='lin')
parser.add_argument("--ensschedtau", default=2000, type=float)
parser.add_argument("--ensschedmin", default=0.0025, type=float)

parser.add_argument("--gradlr",  default=0.1, type=float)
parser.add_argument("--gradsched", choices = ['const', 'lin', 'exp', 'cos'], default='lin')
parser.add_argument("--gradschedpshift", default=0, type=float)
parser.add_argument("--gradschedcycles", default=0.5, type=float)
parser.add_argument("--gradschedtau", default=2000, type=float)
parser.add_argument("--gradschedmin", default=0.01, type=float)

parser.add_argument("--gradwrperiod", default=2000, type=float)
parser.add_argument("--enswrperiod", default=2000, type=float)

parser.add_argument("--gradwrdecay", default=0.5, type=float)
parser.add_argument("--enswrdecay", default=0.75, type=float)
...
from models.rc import FSDDPipelineV7, N_CLASSES
...
train_set, val_set, test_set = fsdd.train_val_test_split(test_size=0.025, val_size=0.075)
...
def log_dist(y, yhat, writer, step):
    ...
    ax.hist(pltr(yhat), bins=np.arange(0, N_CLASSES + 1, 1), label='predicted digit counts')
...
def log_confusion(y, yhat, writer, step):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='magma', 
                xticklabels=np.unique(pltr(y)), yticklabels=np.unique(pltr(y)),
                ax=ax)
...
def testval_log_save(ij, mean_train_l, uncert, saturation, prec, n_free, writer, start_epoch, model, lrs):
    grad_lr, ens_lr = lrs
    i, j = ij
...
def train(start_epoch, opt, esched, gsched, writer):
    ...
    for i in range(start_epoch, args.train2epoch):
        for j, (X,  y) in enumerate(train_gen):
            ...
            testval_log_save((i, j), sum(train_l) / len(train_l), u, opt.n_saturated_params, prec, opt.n_active_weights, writer, start_epoch, opt.forward_model, (opt.grad_lr, opt.ens_lr))
...
if __name__ == "__main__":
    linebreak()
    train_gen = data_utils.DataLoader(train_set, collate_fn=collate_fn, batch_size=args.batch, shuffle=True, pin_memory=True, drop_last=True)
    ...
    uut = FSDDPipelineV7(**model_params).cuda() # V5
    ...
    if args.gradsched == 'const':
        ...
        grad_sched = CosAnnealingSched(args.gradlr, args.gradschedtau, args.gradschedpshift, cycles=args.gradschedcycles,  min_val=args.gradschedmin)
    ...
    ensopt = HybridEnsGradOptimizer(
                            uut, args.nens, args.batch, N_CLASSES, 
                            host_device = 'cuda:0',
                            gpus=[f"cuda:{i}" for i in range(4)],
                            ens_lr = args.enslr,
                            grad_lr = args.gradlr,
                            init_weight_std = args.wstd,
                            noise_sched = UncertAnnealingNS(args.prec, 1e-4, 0.9975),
                            grad_criterion = grad_loss,
                            collapse_policy = 'resampling_with_inflation', #'fixing',
