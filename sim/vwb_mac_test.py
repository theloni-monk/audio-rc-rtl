...
def make_random_bn_onnx(in_dim, out_dim):
    ...
    rmean = onnxhelp.make_tensor(
        f"rmean", TensorProto.FLOAT, [in_dim], np.random.rand(in_dim)
    )
    rvar = onnxhelp.make_tensor(
        f"rvar", TensorProto.FLOAT, [in_dim], np.random.rand(in_dim)
    )

    scale = onnxhelp.make_tensor(
        f"scale", TensorProto.FLOAT, [in_dim], np.random.rand(in_dim)
    )
    bias = onnxhelp.make_tensor(
        f"B", TensorProto.FLOAT, [in_dim], np.random.rand(in_dim)
    )

    bn_node = onnxhelp.make_node(
        "BatchNormalization",
    ...
    grf = onnxhelp.make_graph(
        [bn_node], "testgraph", [X], [Y], [scale, bias, rmean, rvar]
    )
