...
def parse_chain(onnx_subchain, chainname):
    ...
    for idx, node in enumerate(onnx_subchain.graph.node):
        ...
        if node.op_type == "BatchNormalization":
            ...
            curr_mod.wfile = BRAMFile(f"{node.name}_weight", vscaler)
            curr_mod.bfile = BRAMFile(f"{node.name}_bias", effbias)
        elif node.op_type == "MatMul" or node.op_type=="Gemm":
            curr_mod.wfile = BRAMFile(f"{node.name}_weight",
            ...
            if node.op_type=="Gemm":
                curr_mod.bfile = BRAMFile(f"{node.name}_bias",
...
def join_chains(chains: List[ModChain]) -> ModChain:
    pass
...
EPS = 1e-5
def gen_sv_top(onnx_model, initdim,  spec):
